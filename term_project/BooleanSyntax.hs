...
-- Нульместные, одноместные и двуместные операции (связки).
...
-- М.: Физматлит, 2005.
-- В этой книге есть и другие сведения, необходимые для этого проекта.
...
-- Также каждой связке соответствует приоритет. Чем больше приоритет,
-- тем сильнее связывает операция. Приоритеты могут разными в разных
-- учебниках; это только пример.
...
assoc :: Op -> AssocType
-- T, F, Neg не являются бинарными операциями, поэтому их
